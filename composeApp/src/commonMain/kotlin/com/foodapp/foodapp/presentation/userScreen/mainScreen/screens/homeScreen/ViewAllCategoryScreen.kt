...
@Composable
fun ViewAllCategoryScreenRoot(restaurants:List<Restaurant>,viewModel: UserHomeScreenViewModel = koinViewModel()){

    val state by viewModel.uiState.collectAsStateWithLifecycle()
    viewModel.setRestaurantList(restaurants)
    val screenSize = viewModel.getScreenSize()

    ViewAllCategoryScreen(state,onAction = {
        action->
...
@Composable
fun ViewAllCategoryScreen(state:UserHomeScreenState,onAction:(UserHomeScreenAction)->Unit,onBackClick: () -> Unit ={},selectedName:String="Pizza",screenSize:Pair<Int,Int>){
    ...
    Column (modifier = Modifier.fillMaxSize().padding(8.dp)){
        TopAppBar(
            title = {
                Text(text = "Categories", fontSize = TextSize.large, fontWeight = FontWeight.SemiBold, modifier = Modifier.fillMaxWidth().align(Alignment.CenterHorizontally))
            },
            navigationIcon = {
                Icon(
